...
def instance_from_request(request, widget):
        ...
        form_prefix = items.get('form_prefix', None)
        values = {}
        for f in widget.base_field.model._meta.fields:
            if form_prefix:
                name = '%s-%s' % (form_prefix, f.name)
            else:
                name = f.name
            if name in items:
                try:
                    field = f.formfield()
                    if field:
                        value = field.to_python(items[name])
...
class FlexSelectWidget(Select):
    ...
    def _build_js(self, form_prefix=None):
        """
    ...
    def render(self, name, value, attrs=None, choices=(), *args, **kwargs):
        ...
        details() method.
        """
        i = name.rfind('-%s' % self.base_field.name)
        if i != -1:
            form_prefix = name[:i]
        else:
            form_prefix = None
        ...
        html.append(self._build_js(form_prefix=form_prefix))
